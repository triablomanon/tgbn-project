...
def evaluate_model_node_classification(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                       ...
                                       loss_func: nn.Module, num_neighbors: int = 20, time_gap: int = 2000, ma_tracker=None, num_classes: int = None):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if len(eval_idx) > 0:
                ...
                # Update MA features if enabled (temporal progression during evaluation)
                if ma_tracker is not None and model_name in ['DyGFormer']:
                    with torch.no_grad():
                        for idx in eval_idx:
